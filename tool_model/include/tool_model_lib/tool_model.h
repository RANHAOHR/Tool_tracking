...
public:
    ...
    cv::Mat setRandomConfig(double &theta_cylinder, double &theta_oval, double &theta_open);
    void setGErrorToolModel(const cv::Mat &inputparticle, toolModel &generatedTool, cv::Mat &g_error);
    cv::Mat gaussianSampling(const cv::Mat &max_pose);
    ...
    void renderTool(cv::Mat &image, const toolModel &tool, cv::Mat &CamMat, const cv::Mat &P,const cv::Mat &g_error);
